  ...
  <section class="fk-carousel">
    ...
    <Swipeable @swipeLeft="moveTo(selected-1)"  @swipeRight="moveTo(selected+1)" @swiping="swiping" @swipeEnd="moveTo(selected)">
      <div class="fk-carousel__track" :style="styles">
        <img v-for="src,i in slides" :key="i" class="fk-carousel__slide" :src="src" draggable="false" :alt="alt + ' slide ' + (i + 1)"></img>
      </div>
    </Swipeable>
    ...
    <Flex class="fk-carousel__previews" justify="space-evenly">
      <img v-for="src,i in slides" :key="i" class="fk-carousel__preview" :class="{'fk-carousel__preview--active': i === selected}" :alt="alt + ' image ' + (i + 1)" :src="src" @click="moveTo(i)" draggable="false"></img>
    </Flex>
